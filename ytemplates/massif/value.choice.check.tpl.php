...
?>
    <div<?= rex_string::buildAttributes($elementAttributes) ?>>
        <label<?= ($customInputType == 'custom-radios') ? ' class=""' : '' ?>>
            <input value="<?= rex_escape($view->getValue()) ?>" <?= (in_array($view->getValue(), $this->getValue(), true) ? ' checked="checked"' : '') ?> <?= $view->getAttributesAsString() ?> />
            <i class="form-helper<?= ($customInputType == 'custom-radios') ? ' active' : '' ?>"><i></i></i>
