...
class massif_pager extends rex_pager
{
    ...
    {
        if (null === $pageNo) {
            $cursor = (rex_request($this->cursorName, 'int', 0) - 1) * $this->rowsPerPage;
        } else {
            $cursor = $pageNo * $this->rowsPerPage;
        }

        return $this->validateCursor($cursor);
    }
    ...
    {
        $page = rex_request($this->cursorName, 'int', null);
