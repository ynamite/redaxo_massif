 ...
 */
class rex_yform_value_be_table_extended extends rex_yform_value_abstract
{
    protected $fieldData = [];
    ...
    public function enterObject()
    {
        ...
        //dump(json_encode(json_decode($this->getElement('default'), true)));
        if ('' == $this->getValue() && !$this->params['send'] && $this->getElement('default')) {
        ...
        foreach ($_columns as $name => $col) {
            ...
            if ('value' == $col['field']) {
                ...
                $field->init();
                $field->setLabel('');
                if ($col['class'] == 'rex_yform_value_textarea')
                    $field->setElement('attributes', '{"rows":"1"}');
        ...
        if ($this->needsOutput() && $this->isViewable()) {
            if (!$this->isEditable()) {
                ...
                $this->params['form_output'][$this->getId()] = $this->parse('value.be_table_extended.tpl.php', compact('columns', 'data'));
            }
        }
