...
const setValues = () => {
	...
	scrollPadding = parseFloat(computedStyle.getPropertyValue("scroll-padding-top"), 10);
};

const scrollEventThrottle = (fn) => {
	let ticking = false;
	addEvent(window, "scroll.stickyscroll", () => {
		if (!ticking) {
...
const resizeEventThrottle = (fn) => {
	let ticking = false;
	addEvent(window, "resize.stickyscroll", () => {
		if (!ticking) {
			requestAnimationFrame(function () {
				fn();
				ticking = false;
...
const updateInstance = (instance) => {
	let t = (window.scrollY - instance.scrollStart) / (instance.scrollEnd - instance.scrollStart);
