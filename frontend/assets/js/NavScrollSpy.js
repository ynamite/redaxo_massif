...
const NavScrollSpy = () => {
	...
	sections.forEach((section) => {
		let scrollTrigger = ScrollTrigger.create({
			...
			endTrigger: section,
			start: () => {
				const scrollPadding = parseFloat(style.getPropertyValue("scroll-padding-top"), 10) + 5;
				return "top " + scrollPadding + "px";
			},
			end: () => {
				const scrollPadding = parseFloat(style.getPropertyValue("scroll-padding-top"), 10) - 0;
				return "bottom " + scrollPadding + "px";
			},
			onUpdate: function () {
				const menuItems = document.querySelectorAll(`.main-nav [data-menu-item]`);
				const targetNavItems = document.querySelectorAll(`.main-nav [data-menu-item="${section.id}"]`);
