...
class CustomSelect {
	...
	setSelected = () => {
		this.elements.forEach((element) => {
			...
			const selected = select.options[select.selectedIndex]
				? element.wrapper.querySelector('li[data-value="' + select.options[select.selectedIndex].value + '"]')
	...
	selectValue = ($target) => {
		...
		const selectedIndex = element.options.filter((option) => option.value === value)[0].index;
		element.select.selectedIndex = selectedIndex;
		element.select.dispatchEvent(new Event("change"));
		$selectedDiv.innerHTML = value ? `${element.label}: ${$target.innerHTML}` : $target.innerHTML;
	...
	toggleSelect = (event) => {
		...
		const $target = event.target.closest(this.settings.selector + " .custom-select-value-wrap");
		if (!$target) {
	...
	openSelect = async (event) => {
		const $target = event.target.closest(this.settings.selector + " .custom-select");
		if (!$target) return;
	...
	handleKeyboard = (event) => {
		...
		switch (event.code) {
			case "Space":
				$wrap.classList.contains("show") ? $wrap.classList.remove("show") : $wrap.classList.add("show");
				break;
			case "ArrowUp": {
				var prevOption = $options.querySelector(
					'li[data-value="' + $select.options[$select.selectedIndex - 1].value + '"]',
				);
				if (prevOption) {
			...
			default: {
				...
				var $searchedOption = [...$options.querySelectorAll("li")].filter((element) =>
					element.innerHTML.toLowerCase().startsWith(searchTerm) ? element : null,
				);
				if ($searchedOption[0]) {
	...
	bindEvents = () => {
		...
		this.form.addEventListener("click", this.setSelectValue, { signal });
		this.form.addEventListener("click", this.toggleSelect, { signal });
		this.form.addEventListener("blur", this.closeSelect, { capture: true, signal });
		this.form.addEventListener("keydown", this.handleKeyboard, { signal });
	...
	// Release every listener bound by this instance.
	destroy = () => {
		logger.log("CustomSelect", "destroy");
		this.abort.abort();
	};
}
