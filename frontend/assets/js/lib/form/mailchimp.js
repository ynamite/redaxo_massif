...
const AjaxChimp = () => {
	...
	const attach = (form, options = {}) => {
		...
		const fnlabel = fname ? form.querySelector(`label[for="${fname.getAttribute("id")}"]`) : null;
		const lnlabel = lname ? form.querySelector(`label[for="${lname.getAttribute("id")}"]`) : null;
		...
		if (settings.responseDiv) {
			const customLabel =
				typeof settings.responseDiv === "string" ? document.querySelector(settings.responseDiv) : settings.responseDiv;
			if (customLabel) {
				label = customLabel;
				slideUp(label);
			}
		...
		form.addEventListener("submit", (e) => {
			...
			const finalUrl = url + (formData ? "&" : "") + formData + "&language=" + encodeURIComponent(settings.language);
