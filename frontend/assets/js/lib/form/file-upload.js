...
export const fileUpload = async (formElement) => {
	...
	const maxFileSize = input.dataset.maxFileSize ? Math.ceil(input.dataset.maxFileSize / 1000 / 1000) : 10485760;
	const acceptedFiles = input.dataset.acceptedFiles || ".jpg,.png,.gif,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.rar";
	Dropzone.autoDiscover = false;

	const previewTemplateElement = formElement.querySelector("[data-dropzone-template]");
	...
	const dropzone = new Dropzone(formElement, {
		...
		clickable: ".form-group-mupload label, .form-group-mupload .files .clickable", // Define the element that should be used as click trigger to select files.
		dictDefaultMessage: "Dateien hier ablegen oder klicken, um Dateien hochzuladen.",
		dictFallbackMessage: "Ihr Browser unterstützt Drag'n'Drop Datei-Upload nicht.",
		...
		init: function () {
			const self = this;
			const files = JSON.parse(formElement.querySelector("[data-dropzone-files-data]").textContent);

			if (!files.length) return;

			files.forEach((file) => {
				file.accepted = true;
				self.displayExistingFile(file, previewTemplateElement.dataset.url + file.name);
