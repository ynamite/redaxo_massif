 ...
 */
import { fileUpload } from '@/js/lib/form/FileUpload.js'
import { logger } from '@/js/lib/logger/index.js'
import { setFormReadonly, removeAlerts, addAlert } from '@/js/utils/Forms.js'
import { get } from '@/js/lib/store/index.js'

const scrollSmoother = get('scrollSmoother')
...
class Form {
  ...
  initForm = async (form) => {
    ...
    const muploadElements = form.querySelectorAll('.form-group-mupload')
    if (muploadElements.length > 0) {
    ...
    'blur keyup change'.split(' ').forEach(function (event) {
      form.addEventListener(
        event,
        (event) => {
  ...
  processFormData = async (form) => {
    ...
    if (response.error && response.message) {
      ...
      const alerts = addAlertsToForm(response.message, form)
      if (alerts && alerts[0]) {
        scrollSmoother.scrollTo(alerts[0], true, 100)
  ...
  yformHandler = (form) => {
    ...
    if (!$responseForm) {
      ...
      if ($successMessage) {
        ...
        scrollSmoother.scrollTo($successMessage, true, 'top 100px')
      }
    ...
    if ($alert) {
      ...
      $alertEntries.forEach(function (entry, idx) {
        ...
        if (!$input) return
        addAlert({ msg: entry.innerHTML, appendTo: $input })
        if (idx == 0) {
          // get first input, select or textarea element and focus + scroll into view
          const element = $input.querySelector('input, select, textarea')
          setTimeout(() => {
            element.focus()
            // trigger focus event to animate label if needed
            form.dispatchEvent(new Event('focus'))

            // $input.scrollIntoView({ behavior: 'smooth', block: 'center' })
            scrollSmoother.scrollTo($input, true, 'top 100px')
  ...
  initCustomSelect = async (form) => {
    const CustomSelect = (await import('./CustomSelect.js')).default
    this.customSelect = new CustomSelect(form)
    this.customSelect.init()
  }

  initMailChimp = async (form) => {
    const AjaxChimp = (await import('./AjaxChimp.js')).default
    const responseDiv = form.querySelector('.mailchimp-response')
    const ajaxChimp = new AjaxChimp()
    ajaxChimp.init(form, {
      language: 'de',
