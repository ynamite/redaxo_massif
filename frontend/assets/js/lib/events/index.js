...
const disconnectObserver = (label) => {
	...
	if (namespace) {
		...
		if (Object.keys(observers[namespace]).length === 0) delete observers[namespace];
	} else {
...
const disconnectAllObservers = (namespace = null) => {
	if (namespace) {
		...
		for (const label in observers) {
			if (typeof observers[label] === "object" && typeof observers[label]?.disconnect !== "function") {
...
export { addEvent, removeEvent, removeAllEvents, connectObserver, disconnectObserver, disconnectAllObservers };
