 ...
 */
import "./style.css";
import { animateIn as animateIconIn, animateOut as animateIconOut } from "./iconAnimations.js";

import { addEvent, removeAllEvents } from "@/js/lib/events/index.js";
...
class Accordion {
	...
	bindEvents = () => {
		...
		addEvent(document, `click.${self.settings.namespace}`, async (event) => {
			if (event.target.closest("[data-accordion]")) {
				...
				if (!self.settings[id]) self.settings[id] = { state: self.settings.hidden };
	...
	closeOthers = async ($trigger) => {
		const $openItems = $trigger.closest(this.settings.parentSelector).querySelectorAll(".accordion-open");
		if ($openItems.length) {
			$openItems.forEach(($item) => {
				const $itemTrigger = $item.dataset?.accordion ? $item : $item.querySelector("[data-accordion]");
				const id = $itemTrigger.dataset?.accordion;
				const $itemLabel = $itemTrigger.querySelector("span");
				const $itemIcon = $itemTrigger.querySelector(".icon");
				const $itemExp = document.querySelector('[data-accordion-id="' + id + '"]');
				this.closeItem($itemTrigger, $itemLabel, $itemIcon, $itemExp);
			});
		}
	};
	...
	handleCallbacks = async (type, $exp, $trigger) => {
		if (typeof this.settings.callbacks[type] === "function") await this.settings.callbacks[type]($exp, $trigger);
	};
	...
	openItem = async ($trigger, $label, $icon, $exp) => {
		...
		if ($label) {
			...
			if ($trigger.dataset?.accordionToggledLabel) $label.innerHTML = $trigger.dataset.accordionToggledLabel;
		}
		return false;
	};
	...
	closeItem = async ($trigger, $label, $icon, $exp) => {
		...
		if ($label && $trigger.dataset?.accordionLabel) $label.innerHTML = $trigger.dataset.accordionLabel;
