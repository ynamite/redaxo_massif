...
const applyFilter = async (filterValue, elementsToFilter) => {
	...
	for (const element of elementsToFilter) {
		element.originalDisplayStyle = element.originalDisplayStyle || window.getComputedStyle(element).display;
		element.style.display = "none";
	}

	await Flip.from(state, {
		absoluteOnLeave: true,
		onLeave: (elements) => gsap.to(elements, { opacity: 0, scale: 0, duration: 0.4, ease: spring }),
	});
	...
	await Flip.from(finalState, {
		onEnter: (elements) =>
			gsap.fromTo(elements, { opacity: 0, scale: 0 }, { opacity: 1, scale: 1, duration: 0.6, ease: spring }),
	});
...
const init = async (filterWrap, options = {}) => {
	...
	settings.filters = Array.from(filterWrap.querySelectorAll("[data-filter-value]"));
	if (!settings.filters) {
		console.log("Filters not found", `[data-filter-value]`);
		return;
	}
	settings.elementsToFilter = document.querySelector(`[data-filter-elements="${settings.id}"]`).children;
	if (!settings.elementsToFilter) {
		console.log("Filter elements not found", `[data-filter-elements="${settings.id}"]`);
		return;
	}
