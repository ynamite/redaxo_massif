...
export const decryptEmailaddresses = () => {
	...
	document.querySelectorAll('a[href^="javascript:decryptUnicorn"]').forEach(function (element) {
...
export const elementIsVisibleInViewport = (el, partiallyVisible = true) => {
	...
	return partiallyVisible
		? ((top <= viewPortTop && bottom <= viewPortBottom && bottom > viewPortTop) || // bottom is visible and in viewport
				(top >= viewPortTop && bottom <= viewPortBottom) || // is in viewport
				(top >= viewPortTop && bottom >= viewPortBottom && top < viewPortBottom) || // top is visible and in viewport
