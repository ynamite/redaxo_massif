...
(function ($R) {
	...
	const handleFigures = function ({ setCaption = true } = {}) {
		...
		$figures.each(function (figure) {
			const $figure = $(figure);
			let imageComponent = $R.create("image.component", self.app, $figure.find("img"));
			let caption = setCaption ? imageComponent._get_title() : imageComponent._get_caption();
	...
	$R.plugins.image.prototype = Object.assign($R.modules.image.prototype, {
		...
		_insert: async function (data) {
			...
			const result = await fetch(`/redaxo/index.php?rex-api-call=massif_meta_get&filename=${filename}`, {
				method: "GET",
				headers: {
