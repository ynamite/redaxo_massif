	...
	$R.add("plugin", "details", {
		...
		start: function () {
			...
			let obj = {
				...
				icon: false,
				tooltip: redactorTranslations.details_tooltip || "Insert collapsible details",
		...
		_bindEventHandlers: function () {
			...
			if (self._summaryClickHandler) {
				editorElement.removeEventListener("click", self._summaryClickHandler, true);
				editorElement.removeEventListener("mousedown", self._summaryClickHandler, true);
				editorElement.removeEventListener("keydown", self._summaryKeyHandler, true);
			}

			// Create persistent handlers
			self._summaryClickHandler = function (e) {
				var summary = e.target.closest("details[data-details-component] summary");
				if (summary) {
					e.preventDefault();
					e.stopPropagation();
		...
		_onDragOver: function (e) {
			...
			while (target && target !== this.editor.getElement().get()) {
				...
				if (target === this.draggedElement || this.draggedElement.contains(target)) {
					target = target.parentNode;
					continue;
				}

				// Skip if target is inside another details element (prevent nesting)
				if (target.closest("details") && target.closest("details") !== this.draggedElement) {
					target = target.parentNode;
					continue;
				}
		...
		_cleanupEmptyParagraphs: function (element) {
			...
			while (prev) {
				if (prev.nodeType === 1 && prev.tagName === "P") {
					var isEmpty =
						!prev.textContent.trim() || prev.innerHTML.trim() === "<br>" || prev.innerHTML.trim() === "<br/>";
					if (isEmpty) {
						var toRemove = prev;
			...
			while (next) {
				if (next.nodeType === 1 && next.tagName === "P") {
					var isEmpty =
						!next.textContent.trim() || next.innerHTML.trim() === "<br>" || next.innerHTML.trim() === "<br/>";
					if (isEmpty) {
						var toRemove = next;
		...
		_cleanupOrphanedElements: function () {
			...
			for (var i = 0; i < emptyPs.length; i++) {
				var p = emptyPs[i];
				var isEmpty = !p.textContent.trim() || p.innerHTML.trim() === "<br>" || p.innerHTML.trim() === "<br/>";
				var isOrphaned = isEmpty && p.previousSibling && p.previousSibling.tagName === "DETAILS";
      ...
      `;
		...
		_clearDropFeedback: function () {
			...
			var activeElements = this.editor.getElement().get().querySelectorAll(".redactor-drop-active");
